...
export const partnerPaymentApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
    getPartnerPayments: build.query({
      ...
      providesTags: (result = []) =>
        ['PartnerPayment', ...result.map((p) => ({ type: 'PartnerPayment', id: p.id }))],
    }),
    getPartnerPayment: build.query({
      query: (id) => `/partner-payments/${id}`,
      transformResponse: (response) => response?.data,
      providesTags: (_r, _e, id) => [{ type: 'PartnerPayment', id }],
    }),
    updatePartnerPayment: build.mutation({
      ...
      transformResponse: (response) => response?.data,
      invalidatesTags: (_r, _e, { id }) => [{ type: 'PartnerPayment', id }, 'PartnerPayment', 'Dashboard'],
    }),
